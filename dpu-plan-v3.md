...
| 7 | **DPF Operator:** NVIDIA's Kubernetes operator for managing BlueField DPUs -- you apply CRDs (BFB = the firmware/OS image flashed onto the DPU, DPUSet = a group of DPUs to manage together, DPUServiceConfiguration = the network forwarding rules like VLAN tagging and VRF assignment) and the operator configures the DPUs accordingly. |
